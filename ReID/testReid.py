...
def extract_feature_from_img(image, model):
    # No need for batchsize or unsqueezing for a single image
    image = data_transforms(image)  # Assuming data_transforms is defined elsewhere
    model.eval()
    with torch.no_grad():
        features = torch.zeros(linear_num).cuda() if torch.cuda.is_available() else torch.zeros(linear_num)
        for i in range(2):
            if i == 1:
                # Apply horizontal flipping for augmentation
                image = torch.flip(image, dims=[2])
            input_img = Variable(image.unsqueeze(0))  # Adding batch dimension for consistency
            for scale in ms:
                if scale != 1:
                    input_img = torch.nn.functional.interpolate(input_img, scale_factor=scale, mode='bicubic', align_corners=False)
                outputs = model(input_img)
                features += outputs.squeeze()  # Assuming outputs is also 1D, otherwise, modify accordingly
...
def extract_feature_from_path(image):
    # Load and preprocess the image
    ...
    with torch.no_grad():
        features = torch.zeros(linear_num)  # Initialize feature tensor on CPU
        for i in range(2):
            if i == 1:
                # Apply horizontal flipping for augmentation
                image = torch.flip(image, dims=[2])
...
with torch.no_grad():
    ...
    features2 = extract_feature_from_path('two.jpg')
